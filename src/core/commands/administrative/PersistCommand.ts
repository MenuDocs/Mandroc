...
@adminCommand("persist", {
  aliases: [ "persist" ],
  description: {
    content: "Toggles a role's ability to persist when a member with it leaves.",
    usage: "<role> [member]",
...
export class PersistCommand extends MandrocCommand {
  ...
  async exec(message: Message, {
    ...
    if (!member) {
      ...
      embed.setDescription(`${has ? "Removed" : "Added"} ${role} ${has ? "from" : "to"} the list of persisted roles.`);
      await this.persist(has, role.id, KEY);
    } else {
      has = (await this.redis.lrange(`${KEY}:${member.id}`, 0, -1)).includes(role.id);
      embed.setDescription(`${has ? "Removed" : "Added"} ${role} ${has ? "from" : "to"} **${member}'s** list of persisted roles.`);
