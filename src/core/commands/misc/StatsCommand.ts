...
export default class StatsCommand extends MandrocCommand {
  private queries = [
    `https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${config.get<string>("apis.youtube_channelID")}&key=${config.get<string>("apis.youtube_key")}`,
    `https://www.googleapis.com/youtube/v3/search?part=snippet,id&maxResults=5&channelId=${config.get<string>("apis.youtube_channelID")}&key=${config.get<string>("apis.youtube_key")}&order=date`
  ];
