...
export default class ShopCommand extends MandrocCommand {
  public async exec(message: Message) {
      ...
      const embedDescs = [
        ...
        ["XP Boosters", "**1. x1.5**", "**2. x2**", "**3. x2.5**", "x1.5", "x2", "x2.5"],
        ["Change Nickname"]
      ]
...
function makeShopListener(message: Message, descriptions: string[][]) {
  ...
  message.channel
    ...
    .then((collected) => {
      ...
      if (reply.embeds[0].description!.includes("Type any") && reply.content === "back") {
        makeShopListener(message, descriptions);
        return message.channel.send("You cant go any further back - do `cancel` to close the shop!").then(msg => msg.delete({ timeout: 3000 }))
      }

      // Default embed
      if (embed.description?.includes("Type any")) {
        if (!reply.content.match(/^1$|^2$|^3$/)) {
          makeShopListener(message, descriptions);
      ...
      if (embed.title === "Bodyguards" && !reply.content.match(/^1$|^2$|^3$|^4$/)) {
        makeShopListener(message, descriptions);
        return message.channel.send("Please pick a valid number");
      }
