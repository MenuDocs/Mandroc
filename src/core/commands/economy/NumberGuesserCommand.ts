...
export default class NumberGuesserCommand extends MandrocCommand {
  public async exec(message: Message) {
    ...
    async function prompt(): Promise<boolean> {
      ...
      if (guess === num) {
        ...
        const embed = Embed.Success(
          `Correct! Want the amount you guessed :wink: **${num} ₪** has been added to your account!`
        );
      ...
      const embed = Embed.Warning(
        `The number is **${num > guess ? "higher" : "lower"}** than that!`
      );
    ...
    await message.channel.send(
      Embed.Primary("Guess a number between **0** and **500**")
    );
    while (attempts !== 10) {
    ...
    return message.util?.send(
      `Oh no, you've used all 25 of your attempts!\nThe number was **${num}**`
    );
  }
}
