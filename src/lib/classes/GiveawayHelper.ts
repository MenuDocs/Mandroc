...
export abstract class Giveaway {
  static async create(message: Message, duration: number, prize: string, winners: number = 1): Promise<string> {
  ...
  static async end(id: string): Promise<boolean> {
    ...
    const key = TASK("giveaway", id),
      data: ScheduledTaskInfo = (await Scheduler.redis.client.hgetall(key)) as any;
